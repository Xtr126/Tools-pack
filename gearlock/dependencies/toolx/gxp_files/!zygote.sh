...
NAME="GameLoop-Lobby.exe" #Package/Extension Name

CODENAME="gamefool" #An Unique codename

TYPE="Package" #Specify (Package / Extension)

AUTHOR="LoLMan" #Your name as the Developer/Owner/Packer
...
REQSYNC="no" #(Deafult - yes) To make sure all your files are properly written in disk
...
DEF_HEADER="no" #(Default - yes) Whether to use the default header which print's the info during zygote
